...
function MyGroupScreen({ navigation }) {
    ...
    useEffect(() => {
        ...
        .then(
            async (userDoc) => {
                const listOfGroups = await userDoc.data().group;
                if (listOfGroups == null) return;
                const holder = [];
                await Promise.all(listOfGroups.map(async (groupId) => {
                    const loading2 = await groupRef.doc(groupId).get().then(async (doc) => {
